# CodeX 5.5 medium 実行プロンプト

> このファイルの内容をそのまま CodeX 5.5 medium に貼り付けて実行する。
...
2. **各フェーズ完了ごとに `npx tsc --noEmit && pnpm lint` を実行**し、エラー 0 を確認してから次フェーズへ進む
...
1. 作成したファイル一覧
2. 変更したファイル一覧
3. `npx tsc --noEmit && pnpm lint && pnpm build` の結果
4. ユーザーに以下の目視確認を依頼:
